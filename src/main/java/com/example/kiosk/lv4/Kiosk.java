    ...
    public void start() {
        ...
        while (running) {
            ...
            for (int i = 0; i < menuCategoryList.size(); i++) {
                System.out.printf("%d. %s%n", i + 1, menuCategoryList.get(i).categoryName);
            }
            ...
            if (categoryChoice == 0) {
                ...
                while (inCategory) {
                    System.out.println("\n[ " + selectedCategory.categoryName.toUpperCase() + " MENU ]");
                    List<MenuItem> items = selectedCategory.menuList;
                    for (int i = 0; i < items.size(); i++) {
                        MenuItem item = items.get(i);
                        System.out.printf("%d. %-15s | ₩ %.1f | %s%n", i + 1, item.name, item.price, item.description);
                    }
